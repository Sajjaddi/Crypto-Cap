...
const CoinItem = ({ data, index, last }) => {
  const options = {
    chart: {
      toolbar: {
        show: false,
  ...
  return (
    <>
      <li className="lg:hidden bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.05)] rounded-[18px] px-5">
      ...
      <li className={`${last && 'rounded-b-[4px]'} hidden lg:grid bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.05)] lg:items-center lg:grid-cols-12 text-[15px] text-tara-fifth px-6`}>
        ...
        <div className="col-start-2 col-end-7 flex items-center justify-start gap-x-4">
          ...
          <span className="border-l border-[#403A3A] pl-3 text-sm">
            {data?.symbol?.toUpperCase()}
          </span>
        </div>
        <span className="font-roboto col-start-7 col-end-8">
          ${data?.current_price?.toLocaleString()}
        </span>
        <span className="font-roboto col-start-9 col-end-10">
          {data?.price_change_percentage_24h}%
        </span>
        <div className="col-start-10 mx-auto col-end-12 w-24 h-16 flex justify-center">
          <Chart
            series={series}
            options={options}
            type="line"
